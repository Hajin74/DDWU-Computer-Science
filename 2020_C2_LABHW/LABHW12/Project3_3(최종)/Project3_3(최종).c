...
void purchaseVideo(VideoInfo videoList[], int* videoCountPtr, char* title, int qty) { // 구입
...
int searchVideoByTitle(VideoInfo* videoList, int videoCount, char* title) {
	...
	for (i = 0; i < videoCount; i++) {
		if (strcmp(videoList[i].title, title) == 0) { // 동일. 영화가 있다.
			if (videoList[i].qty >= 1) { // 영화가 1개이상이면
				flag = i; // 배열 인덱스를 반환
				return flag;
			}
		}
...
void rentVideo(VideoInfo *videoList, int *videoCount, RentInfo *rentList, int *rentCount, char *title, int custId) {
	...
	index = searchVideoByTitle(videoList, videoCount, title);
